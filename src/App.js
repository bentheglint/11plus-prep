...
import {
  ...
  RightTriangleExteriorAngle, CoordinateGrid, VennDiagram, NetDiagram
...
function App({ currentUser: authUser, getToken, loadedData, activeChildId: initialChildId, childrenList: initialChildrenList, userEmail, tutorEligible, isAdmin, entitlement, clearJoinCodeMetadata }) {
  ...
  // This is SEPARATE from the /join/<code> flow — invite tokens are one-time links
  // generated by the bulk-invite backend, not reusable tutor codes.
  const [pendingInviteToken, setPendingInviteToken] = useState(() => {
    try { return sessionStorage.getItem('pending-invite-token') || null; } catch { return null; }
  });
...
Remember: This is a child learning. Be warm and make learning fun — but the learning only happens if they do the thinking themselves.`;
    ...
    return (
      <InviteClaimScreen
        ...
        onClaimed={(childId) => {
          ...
          try { sessionStorage.removeItem('invite-preview'); } catch {}
  ...
  if (currentView === 'bulkInvite') {
    return (
      <BulkInviteScreen
        getToken={getToken}
        onBack={() => setCurrentView('tutorDashboard')}
      />
    );
  }
  ...
  if (currentView === 'tutorDashboard') {
    return (
      <TutorDashboardScreen
        ...
        onBulkInvite={() => setCurrentView('bulkInvite')}
