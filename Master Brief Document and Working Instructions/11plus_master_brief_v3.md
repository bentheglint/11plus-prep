# 11+ EXAM PREP APP - MASTER BRIEF v3.0
...
- 🚧 **IMMEDIATE PRIORITY:** Add image/diagram support to app
- 🎯 Target: Continue with Medium Priority topics after image support added
- 📅 Timeline: 8 months until exam (September 26, 2026)

---

## CRITICAL - IMAGE SUPPORT REQUIREMENT

### Why This Is Essential Now

**Discovery:** GL Assessment questions frequently include visual elements:
- Bar charts and graphs (data questions)
- Geometric shapes and diagrams (area, perimeter, angles)
- Tables and grids (coordinate geometry, data)
- Clock faces (time questions)
- Spinners (probability)
- Number lines (negative numbers, sequences)

**Current Limitation:** All 290 questions generated so far are text-only.

**Decision Made:** PAUSE question generation until image support is added to app.

### Implementation Requirements

**1. Question Structure Enhancement**
```javascript
{
  id: 1,
...
- **SVG format** for programmatic generation (scalable, crisp, small file size)
- Claude can generate SVG code for:
  - Simple geometric shapes with measurements
  - Bar charts and simple graphs
  - Coordinate grids
  - Number lines
  - Basic diagrams
- Avoids manual image creation (essential for 100s-1000s of questions)

**4. App Code Changes Needed**
- Modify question display component to check for image field
- Render image when present
- Maintain responsive sizing
- Keep working without images when not present (backward compatible)

### Next Steps (IN ORDER)
1. **Update React app** to support optional image field
2. **Test with sample images** to verify rendering
3. **Resume question generation** with image support
4. **Review existing 290 questions** - identify which would benefit from adding images
