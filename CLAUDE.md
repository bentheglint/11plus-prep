...
- **Email (mailboxes):** Google account = **prepstepapp@gmail.com** (the login; this is what the Gmail MCP connects to). **hello@prepstep.co.uk** = the Google Workspace / send-as alias to select in Gmail's From dropdown when sending (resolved with Ben, 16 Jul 2026). So: draft into prepstepapp@gmail.com, send AS hello@prepstep.co.uk. ⚠ App's Resend `EMAIL_FROM` exact value still unconfirmed (likely an @prepstep.co.uk address; Resend can't verify a gmail.com sender).
...
- **Analytics:** Cloudflare Web Analytics (cookieless), NOT GA4. CF account `d2ab95cc060e1c5684627a0f46ff1a22`. ⚠ prepstep.co.uk collects under TWO live site tags (`ec5299682ab546c18a47cbfa29cc5fbb` + `aa7bd4c8287c40cd9dfb02efcb077a68`) — a duplication to consolidate; confirm which the live site embeds. The old documented tag `798fdc0…` is STALE (collects nothing). Pull real traffic via `node scripts/pull-cf-traffic.mjs 30` (read-only Account-Analytics token in gitignored `scripts/.metrics.env`). Numbers snapshot in `.agents/metrics.md`.
...
- **MCP connectors (machine-specific):** "claude.ai Gmail" authed for ben@venortech.com on home PC + work PC; on the LAPTOP authed to **prepstepapp@gmail.com** (the PrepStep account, 16 Jul) for branded PrepStep drafts. ONE account at a time per machine, re-auth via `/mcp` to switch. Draft/read/label only, NO send tool (and no delete-draft tool). Chrome DevTools MCP (Visual QA).
