#!/usr/bin/env node
// Insert verified Maths D3 elite multi-step wave (benchmark fix #8) into mathsData.js.
// Spec: research/maths-d3-multistep-spec.md. Mirrors insert-vr7.mjs conventions:
// dry-run by default, seeded position-balance of `correct` A-E, CRLF-safe append,
// string-aware bracket matching (so existing content is never disturbed), and a
// post-write vm/module-load validation pass.
//
// Input: JSON shaped { "<topicKey>": [ item, ... ], ... } — same shape verify-maths-d3.mjs
// checks. Each item carries the normal question fields plus _expr/_steps/_distractors,
// which are stripped on insert (never written to mathsData.js).
//
// For EACH topic key present in the input, this locates `<key>: {` then the FIRST
// `questions: [` after it, then finds the MATCHING closing `]` with a string-aware
// scanner (tracks [] / {} depth, skips over string literals including escaped quotes)
// so brackets inside explanation text can never break the match. New items are
// appended (comma-prefixed) immediately before that `]`, ids continuing from the
// current max id in that topic, and `correct` position-balanced A-E per topic using
// a fixed seed.
//
// Usage:
//   node scripts/data-generation/insert-maths-d3.mjs                  (dry-run)
//   node scripts/data-generation/insert-maths-d3.mjs --apply          (writes real file)
//   node scripts/data-generation/insert-maths-d3.mjs --data <path> --target <path> [--apply]
...
const SEED = 20260722; // fixed constant (today's authoring date) — deterministic, reproducible
...
const dataPath = argVal('--data', path.join(SCRATCHPAD, 'maths-d3-clean.json'));
...
const lessonMapPath = argVal('--lessonmap', path.join(SCRATCHPAD, 'maths-d3-lessonmap.json'));
...
// ---------- helpers (copied from insert-vr7.mjs conventions) ----------
...
async function main() {
  ...
  for (const topic of TOPIC_ORDER) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!Array.isArray(it.options) || it.options.length !== 5) {
        errors.push(`GUARD [${topic}] item #${i + 1}: options must be an array of 5 (run verify-maths-d3.mjs first)`);
      }
      if (!Number.isInteger(it.correct) || it.correct < 0 || it.correct > 4) {
        errors.push(`GUARD [${topic}] item #${i + 1}: correct must be an integer 0..4 (run verify-maths-d3.mjs first)`);
      }
    }
  ...
  console.log(`=== INSERT MATHS D3 — ${apply ? 'APPLY' : 'DRY-RUN'} ===`);
