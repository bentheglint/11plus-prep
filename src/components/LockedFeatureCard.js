...
// CLAUDE.md).
function LockedFeatureCard({ title, description, onUpgrade, className = '', showCta = true }) {
  return (
    <div className={`card rounded-2xl p-8 text-left flex flex-col border-2 border-gray-200 ${className}`}>
      ...
      {showCta ? (
        ...
        <span className="self-start inline-flex items-center gap-1.5 text-sm font-bold text-gray-400 bg-gray-100 rounded-full px-4 py-2">
