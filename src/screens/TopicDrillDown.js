      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="card-elevated p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <h2 className="text-2xl font-heading font-bold text-slate-800">{displayName}</h2>
            </div>
            <button
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-xl font-heading font-bold text-slate-800">{m.totalQuestions}</p>
              <p className="text-[10px] text-slate-500">Questions</p>
            </div>
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-xl font-heading font-bold text-slate-800">{m.daysSince === Infinity ? '—' : `${m.daysSince}d`}</p>
              <p className="text-[10px] text-slate-500">Last practised</p>
            </div>
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-xl font-heading font-bold text-slate-800">{avgTimeSecs ? `${avgTimeSecs}s` : '—'}</p>
            ...
            <svg width="100%" viewBox={`0 0 ${sparkLeft + sparkW + 35} ${sparkTop + sparkH + 20}`}>
              ...
              {[0, 25, 50, 75, 100].map(pct => (
                <g key={pct}>
                  ...
                  <text x={sparkLeft + sparkW + 5} y={sparkTop + sparkH - (pct / 100) * sparkH + 4} fontSize="8" fill="#64748B">{pct}%</text>
                </g>
              ))}
              {/* Line */}
        ...
        {diffData.some(d => d.total > 0) && (
          <div className="card-elevated p-5 mb-6">
              ...
              {diffData.map(d => {
                if (d.total === 0) return (
                  <div key={d.difficulty} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-slate-500">No questions yet</span>
                  </div>
                );
                return (
                  <div key={d.difficulty}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-slate-500">{d.correct}/{d.total} ({d.pct}%)</span>
                    </div>
            ...
            {diffData[0].total > 0 && diffData[2].total > 0 && diffData[2].pct < diffData[0].pct - 20 && (
              <p className="text-xs text-slate-500 mt-3 p-2 bg-amber-50 rounded-lg">
              ...
              {topicResults.slice(0, 20).map((r, i) => {
                ...
                return (
                  <div key={i} className={`flex items-center gap-3 py-1.5 px-2 rounded text-xs ${r.correct ? 'bg-green-50' : 'bg-red-50'}`}>
                    ...
                    <span className="text-slate-800 flex-1 truncate">Q{r.questionId}</span>
                    {timeSecs && (
                      <span className="text-slate-500 flex-shrink-0">{timeSecs}s</span>
                    )}
                  </div>
                );
              })}
        ...
        {topicResults.length === 0 && (
          <div className="card-elevated p-8 text-center mb-6">
            ...
            <h3 className="font-heading font-bold text-slate-800 mb-2">No data yet</h3>
            <p className="text-sm text-slate-500 mb-4">Complete some questions on this topic to see your detailed stats.</p>
            <button
              onClick={() => onPractise(subject, topicKey)}
