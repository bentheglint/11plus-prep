...
Remember: This is a child learning, so be warm, supportive, and make learning fun!`;
  ...
  // === LESSON VIEW (merged master + sub-concepts) ===
  // lessonChoice and subConceptList views removed — all lessons now served via selectLesson()
  //
  /* subConceptList view removed — staging merged into lessonBank */
  if (false && currentView === 'subConceptList') {
    const topicName = lessonBank[selectedTopic]?.name || selectedTopic;
    const subConcepts = [];

    // Group by category
    const grouped = {};
    const categoryOrder = ['core', 'supporting', 'extension', 'other'];
    ...
    const categoryColours = {
      core: { bg: 'bg-[#0984E3]/10', text: 'text-[#0984E3]' },
      supporting: { bg: 'bg-[#00B894]/10', text: 'text-[#00B894]' },
      extension: { bg: 'bg-[#6C5CE7]/10', text: 'text-[#6C5CE7]' },
      other: { bg: 'bg-[#636E72]/10', text: 'text-[#636E72]' },
    };

    // Progress tracking helpers
    const testedKey = (scId) => `${selectedTopic}:${scId}`;
    const isTested = (scId) => !!testedSubConcepts[testedKey(scId)];
    const testedCount = subConcepts.filter(sc => isTested(sc.id)).length;

    const toggleTested = (e, scId) => {
      e.stopPropagation();
      const key = testedKey(scId);
      const updated = { ...testedSubConcepts };
      if (updated[key]) {
        delete updated[key];
      } else {
        updated[key] = true;
      }
      setTestedSubConcepts(updated);
      localStorage.setItem('tested-subconcepts', JSON.stringify(updated));
    };

    const handleSubConceptClick = (sc) => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#DFE6E9] via-[#FFEAA7]/20 to-[#DFE6E9] p-6">
        <div className="max-w-2xl mx-auto">
          <button
            onClick={() => setCurrentView('lessonChoice')}
            className="flex items-center gap-2 text-[#636E72] hover:text-[#2D3436] mb-6 transition-colors"
          >
          ...
          <div className="text-center mb-8 animate-fade-in-up">
            <h2 className="text-2xl font-heading font-bold text-[#2D3436] mb-1">
              {topicName}
            </h2>
            <p className="text-[#636E72]">Sub Concepts — {testedCount}/{subConcepts.length} checked</p>
            <div className="w-48 mx-auto mt-3 h-2 bg-[#DFE6E9] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#00B894] rounded-full transition-all duration-300"
                style={{ width: `${subConcepts.length ? (testedCount / subConcepts.length) * 100 : 0}%` }}
              />
            </div>
          </div>
          ...
          <div className="space-y-6">
            {sortedCategories.map(cat => {
              const colours = categoryColours[cat] || categoryColours.other;
              const catTested = grouped[cat].filter(sc => isTested(sc.id)).length;
              return (
                <div key={cat}>
                  <h3 className="text-sm font-semibold text-[#636E72] uppercase tracking-wider mb-3">
                    {cat} ({catTested}/{grouped[cat].length})
                  </h3>
                  <div className="space-y-2">
                    {grouped[cat].map(sc => {
                      const checked = isTested(sc.id);
                      return (
                        <div
                          key={sc.id}
                          className={`card w-full rounded-xl px-5 py-4 text-left hover:scale-[1.01] transition-all border-2 flex items-center gap-3 ${checked ? 'border-[#00B894]/30 bg-[#00B894]/5' : 'border-transparent hover:border-[#6C5CE7]/20'}`}
                        >
                          <button
                            onClick={(e) => toggleTested(e, sc.id)}
                            className={`flex-shrink-0 w-7 h-7 rounded-lg border-2 flex items-center justify-center transition-all ${checked ? 'bg-[#00B894] border-[#00B894] text-white' : 'border-[#B2BEC3] hover:border-[#00B894]'}`}
                          >
                            {checked && <CheckCircle className="w-4 h-4" />}
                          </button>
                          <button
                            onClick={() => handleSubConceptClick(sc)}
                            className={`flex-1 text-left flex items-center justify-between min-w-0 ${checked ? 'opacity-70' : ''}`}
                          >
                            <span className={`font-medium ${checked ? 'text-[#636E72] line-through' : 'text-[#2D3436]'}`}>{sc.name}</span>
                            <span className={`text-xs font-semibold px-2 py-1 rounded-full flex-shrink-0 ml-3 ${colours.bg} ${colours.text}`}>
                              {cat}
                            </span>
                          </button>
                        </div>
                ...
                <div className="mb-6">
                  <div className={selectedTopic === 'hiddenWords' ? 'flex flex-wrap justify-center gap-2' : 'grid grid-cols-2 gap-3 sm:grid-cols-3'}>
                    {currentQuestion.options.map((option, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleSelectTwoToggle(idx)}
              ...
              {showTutorChat && (
                <div className="mt-6 bg-[#EDE8FF] border-2 border-[#A29BFE]/40 rounded-xl p-4 animate-fade-in-up">
                  ...
                  <div className="flex gap-2">
                    ...
                    {speechSupported && (
                      <button
                        onClick={() => toggleListening(setUserMessage)}
                        disabled={isAiThinking}
        ...
        <div className="max-w-2xl mx-auto">
          <div className="card-elevated p-6 md:p-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 stagger-children">
              ...
              <button
                onClick={() => setCurrentView(quizMode === 'daily' ? 'learningMode' : 'topics')}
